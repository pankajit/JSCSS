...
/* Moved code from cartorderdetails.phtml to JS File */
jQuery(document).ready(function() {
    var selectVal = jQuery('.order_type :selected').val();
    var selectDeliveryType = jQuery('.delivery_type :selected').val();
    jQuery("#ship_via").html(selectDeliveryType);
    if(selectVal == truckloadorder) {
        jQuery( ".payment_terms_stock" ).hide();
        jQuery( ".payment_terms_truckload" ).show();
    } else if(selectVal == stockorder){
        jQuery( ".payment_terms_truckload" ).hide();
        jQuery( ".payment_terms_stock" ).show();
    }


    // Condition releated jQuery
        if(!jQuery('#shipdatevalidate').val()) {
                //changeDeliveyDate();
            }

    jQuery( ".jsobserve" ).change(function() {
        if(this.name == 'order_type') {
            var selectVal = jQuery('.order_type :selected').val();
            if(selectVal == truckloadorder) {
                jQuery( ".payment_terms_stock" ).hide();
                jQuery( ".payment_terms_truckload" ).show();
    ...
    if(jQuery('#addtocartvia').length > 0) {
      var addtocartvia = jQuery('#addtocartvia').val();

      if(!addtocartvia) {
        callDisableAddtocartvia();
      }
    }
});
...
function validateOrderType(){
  ...
  function validateSingleQty(obj){
    ...
    var check_qty = parseFloat(obj.value).toFixed(2);
      if(check_qty > 0 && check_qty % 1 === 0)
      {
	  if(check_qty > 10000){
	   validationstr = '<div class="validation-advice" id="advice-required-entry-qty" style="">Quantity exceeds the maximum allowable (10,000).</div>';
	   jQuery('#validation_'+eleid).html(validationstr);
          validflag = false;
	  }else{
          jQuery('#validation_'+eleid).html('');
      ...
      }else
      {
          if(check_qty == 0.00){
              ...
              if(qty == ''){
                  ...
                  var regx = /^[a-zA-Z]+$/;
                  if (regx.test(check_qty)){
                      var validationstr = '<div class="validation-advice" id="advice-required-entry-qty" style="">'+alpha_qty+'</div>';
                  }else{
